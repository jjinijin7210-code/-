...
// 2026-07-23: 진희님 본인 유튜브 채널(트롯충전소·감성채널·코코로의 모구모구식당)용 쇼츠 대본 -
// SNS 캡션이 아니라 실제 소리 내어 읽는 "나레이션 대본"이라 문체가 달라야 함. 강렬한 후킹으로
// 시작하도록 못박음(psychologyScript.js의 STRONG_HOOK_RULE과 같은 취지, 여기선 범용 톤이라
// 별도로 둠).
const YOUTUBE_SHORTS_TONE = `유튜브 쇼츠 대본 톤: 이건 SNS 캡션이 아니라 실제로 소리 내어 읽는
나레이션 대본이야. 반드시 아래 구조로 써:
...
2. 본론: 짧고 리듬감 있는 문장으로, 실제 사람이 말하듯 자연스럽게(문어체 금지)
3. 마무리: 다음 영상을 기대하게 만들거나, 구독/좋아요를 자연스럽게 유도하는 한 줄
본문(body)은 실제 촬영/녹음할 때 그대로 읽을 수 있는 대본 형태로 써줘 (화면 지시문이나
효과음 표시 없이, 말하는 내용만).`
